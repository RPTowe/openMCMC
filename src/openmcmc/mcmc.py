...
class MCMC:
    ...
    def __post_init__(self):
        ...
        """
        if self.model.check_state(self.state):
            raise ValueError("Error: initial state ill defined.")
        ...
        if self.model.response is not None:
            for response_name, value in self.model.response.items():
                self.store[response_name] = np.full(
                    shape=(self.state[value["response"]].size, self.n_iter), fill_value=np.nan
                )
